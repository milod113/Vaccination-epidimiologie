...
void main() {
  testWidgets('Plateforme Vaccination welcome dashboard renders', (tester) async {
    await di.sl.reset();
  ...
  testWidgets('Welcome dashboard renders on a narrow phone viewport', (tester) async {
    await di.sl.reset();
    await di.initDependencies();
  ...
  testWidgets('Tétanos dashboard layout renders with premium sidebar', (tester) async {
    await di.sl.reset();
    await di.initDependencies();
  ...
  testWidgets('Antirabique dashboard layout uses shared premium sidebar', (tester) async {
    await di.sl.reset();
    await di.initDependencies();
    tester.view.physicalSize = const Size(1440, 1024);
    tester.view.devicePixelRatio = 1.0;
    ...
    await tester.pumpWidget(const MaterialApp(
      home: AntirabiqueDashboardLayout(),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('Tétanos evaluation screen renders wide (desktop layout)', (tester) async {
    tester.view.physicalSize = const Size(1280, 1000);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.reset);

    await tester.pumpWidget(const MaterialApp(
      home: TetanusEvaluationScreen(),
    ));
    await tester.pumpAndSettle();
    ...
    await tester.scrollUntilVisible(find.text('Niveau de risque'), 300,
        scrollable: contentList);
    expect(find.text('Niveau de risque'), findsOneWidget);
    await tester.scrollUntilVisible(find.text('Décision clinique'), 400,
        scrollable: contentList);
    expect(find.text('Décision clinique'), findsOneWidget);
    await tester.scrollUntilVisible(find.text('Synthèse clinique'), 800,
        scrollable: contentList);
    expect(find.text('Synthèse clinique'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });

  testWidgets('Tétanos evaluation screen renders narrow (mobile)', (tester) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.reset);
  ...
  testWidgets('Tétanos case detail renders wide (clinical synthesis)', (tester) async {
    await di.sl.reset();
    await di.initDependencies();
    tester.view.physicalSize = const Size(1280, 1000);
    tester.view.devicePixelRatio = 1.0;
    ...
    await tester.pumpWidget(const MaterialApp(
      home: TetanusPatientDetailScreen(patientId: 'TET-004'),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('Tétanos case detail renders narrow (sections scrollable)', (tester) async {
    await di.sl.reset();
    await di.initDependencies();
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1.0;
    ...
    await tester.pumpWidget(const MaterialApp(
      home: TetanusPatientDetailScreen(patientId: 'TET-002'),
    ));
    await tester.pumpAndSettle();
