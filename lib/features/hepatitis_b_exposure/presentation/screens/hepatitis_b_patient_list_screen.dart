...
import '../widgets/hep_b_patient_card.dart';

class HepatitisBPatientListScreen extends StatefulWidget {
  const HepatitisBPatientListScreen({super.key});

  @override
  State<HepatitisBPatientListScreen> createState() =>
      _HepatitisBPatientListScreenState();
}

class _HepatitisBPatientListScreenState
    extends State<HepatitisBPatientListScreen> {
  final _repo = GetIt.instance<HepBRepository>();
  late List<HepBExposurePatient> _patients;
  List<HepBExposurePatient> _filtered = [];
    ...
    setState(() {
      _filtered = _patients.where((p) {
        final matchesSearch = q.isEmpty ||
            ...
            p.typeExposition.label.toLowerCase().contains(q);
        final matchesStatut = _filterStatut == 'Tous' ||
            p.statutDossier.label == _filterStatut ||
            (_filterStatut == 'Urgent' && p.niveauRisque == HepBRiskLevel.urgent);
  ...
  int get _urgentCount =>
      _patients.where((p) => p.niveauRisque == HepBRiskLevel.urgent).length;
  int get _enCoursCount =>
      _patients.where((p) => p.statutDossier == HepBDossierStatut.enCours).length;
  int get _termineCount =>
      _patients.where((p) => p.statutDossier == HepBDossierStatut.suiviTermine).length;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text('Liste des cas', style: GoogleFonts.inter(
  ...
  Widget _buildStatsBar() {
    return Container(
      ...
      child: Row(
        children: [
          _statPill('${_patients.length}', 'Total', EpidemiologyTheme.indigo),
          const SizedBox(width: 8),
          _statPill('$_urgentCount', 'Urgent', EpidemiologyTheme.danger),
          const SizedBox(width: 8),
          _statPill('$_enCoursCount', 'En cours', EpidemiologyTheme.warning),
          const SizedBox(width: 8),
          _statPill('$_termineCount', 'Terminé', EpidemiologyTheme.success),
        ],
      ),
    );
  }
          ...
          child: Builder(builder: (context) {
            ...
            return AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: hasFocus
                      ? EpidemiologyTheme.indigo.withValues(alpha: 0.5)
                      : EpidemiologyTheme.warm100,
                  width: hasFocus ? 1.5 : 1,
                ),
                boxShadow: hasFocus
                    ? [BoxShadow(
                        color: EpidemiologyTheme.indigo.withValues(alpha: 0.08),
                        blurRadius: 12,
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Rechercher un patient (nom, ID, exposition)…',
                  hintStyle: TextStyle(
                    color: EpidemiologyTheme.warm400,
                  ...
                  suffixIcon: _searchController.text.isNotEmpty
                      ? IconButton(
                          icon: Icon(Icons.close_rounded,
                            size: 20, color: EpidemiologyTheme.warm400),
                          onPressed: () {
                            _searchController.clear();
                            _searchFocus.unfocus();
                          })
                ...
                style: TextStyle(
                  fontSize: 14, color: EpidemiologyTheme.warm800,
  ...
  Widget _buildFilterRow() {
    final statuts = [
      _FilterOption('Tous', null, null),
      _FilterOption('En cours', null, HepBDossierStatut.enCours),
      _FilterOption('Urgent', HepBRiskLevel.urgent, null),
      _FilterOption('Terminé', null, HepBDossierStatut.suiviTermine),
      _FilterOption('Perdu de vue', null, HepBDossierStatut.perduDeVue),
    ];
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: statuts.map((opt) {
            ...
            int count;
            if (opt.riskLevel != null) {
              count = _patients
                  .where((p) => p.niveauRisque == opt.riskLevel)
                  .length;
            } else if (opt.statut != null) {
              count = _patients
                  ...
                  decoration: BoxDecoration(
                    gradient: active
                        ? LinearGradient(
                            colors: [
                              EpidemiologyTheme.indigo,
                              EpidemiologyTheme.indigo.withValues(alpha: 0.8),
                            ],
                            begin: Alignment.topLeft,
                            end: Alignment.bottomRight,
                          )
                        : null,
                    color: active ? null : EpidemiologyTheme.warm50,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) => _AnimatedListItem(
        ...
        child: Padding(
          ...
          child: HepBPatientCard(patient: _filtered[i]),
        ),
      ),
    );
  }
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: EpidemiologyTheme.warm100, width: 1,
                ),
              ),
              child: Center(
                child: Container(
            ...
            Text(
              hasSearch
                  ...
                  : 'Les patients pris en charge pour une\nexposition VHB apparaîtront ici',
              textAlign: TextAlign.center,
              style: GoogleFonts.inter(
                fontSize: 13, color: EpidemiologyTheme.warm400,
                height: 1.6,
              ),
            ),
            if (!hasSearch) ...[
              ...
              FilledButton.icon(
                ...
                style: FilledButton.styleFrom(
                  backgroundColor: EpidemiologyTheme.indigo,
...
class _FilterOption {
  final String label;
  final HepBRiskLevel? riskLevel;
  final HepBDossierStatut? statut;
  const _FilterOption(this.label, this.riskLevel, this.statut);
}
